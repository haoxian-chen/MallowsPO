...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.home_advantage = args.home_advantage
        self.length_normalization = args.length_normalization
        self.dispersion_mean = args.neg_log_dispersion_mean
        self.if_mixing_alpha = args.if_mixing_alpha
        self.mixing_alpha = args.mixing_alpha
        self.add_sft_loss = args.add_sft_loss
        self.sft_coef = args.sft_coef

        if not self.add_sft_loss:
            self.sft_coef = 0.0

        if self.reference_free:
            self.mixing_alpha = 0.0

        self.output_reference_entropy = args.output_reference_entropy

        if self.output_reference_entropy:
            self.all_reference_entropy = []
            self.output_reference_entropy_local_dir = args.output_reference_entropy_local_dir
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if self.precompute_ref_log_probs and not self._precomputed_train_ref_log_probs:
            ...
            for padded_batch in tqdm(iterable=data_loader, desc="Train dataset reference log probs"):
                reference_chosen_logp, reference_rejected_logp, reference_entropy = self.compute_reference_log_probs(padded_batch)
                reference_chosen_logp, reference_rejected_logp, reference_entropy = self.accelerator.gather_for_metrics(
                    (reference_chosen_logp, reference_rejected_logp, reference_entropy)
                )
                reference_chosen_logps.append(reference_chosen_logp.cpu())
                reference_rejected_logps.append(reference_rejected_logp.cpu())
                reference_entropies.append(reference_entropy.cpu())

            all_reference_chosen_logps = torch.cat(reference_chosen_logps).float().numpy()
            all_reference_rejected_logps = torch.cat(reference_rejected_logps).float().numpy()
            all_reference_entropies = torch.cat(reference_entropies).float().numpy()
            ...
            if self.output_batches_entropy:
                np.save(self.output_batches_entropy_local_directory, all_reference_entropies)

                print("Finish saving the reference entropies.Quiting here for debugging and compute mean")
                sys.exit()

            self.train_dataset = self.train_dataset.add_column(
                name="reference_entropies", column=all_reference_entropies
            )
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        ...
        if self.precompute_ref_log_probs and not self._precomputed_eval_ref_log_probs:
            ...
            for padded_batch in tqdm(iterable=data_loader, desc="Eval dataset reference log probs"):
                reference_chosen_logp, reference_rejected_logp, _ = self.compute_reference_log_probs(padded_batch)
    ...
    def compute_reference_log_probs(self, padded_batch: Dict) -> Dict:
        ...
        return reference_chosen_logps, reference_rejected_logps, reference_entropies
    ...
    def dpo_loss(
        ...
        reference_entropy: torch.FloatTensor,
        ...
        if self.if_mixing_alpha:
            logits = pi_logratios - self.mixing_alpha * ref_logratios - (1.0-self.mixing_alpha) * self.home_advantage
        else:
            logits = pi_logratios - ref_logratios - self.home_advantage
        ...
        if self.loss_type == "sigmoid":
            ...
            losses = (
                - ((self.beta * logits).atan()/math.pi + 0.5).log()
            )
        ...
        elif self.loss_type == "mallows_dpo":
            # self.dispersion_mean = torch.tensor(self.dispersion_mean, dtype=torch.float32, requires_grad=False)
            self.neg_log_dispersion = - self.dispersion_mean * torch.log(reference_entropy)
            # self.neg_log_dispersion.no_grad()
            losses = (
                -F.logsigmoid(self.beta * logits * self.neg_log_dispersion) * (1 - self.label_smoothing)
                - F.logsigmoid(-self.beta * logits * self.neg_log_dispersion) * self.label_smoothing
            )
        elif self.loss_type == "mallows_ipo":
            # self.dispersion_mean = torch.tensor(self.dispersion_mean, dtype=torch.float32, requires_grad=False)
        ...
        if self.add_sft_loss:
            # chosen_logratios = policy_chosen_logps - reference_chosen_logps
            # losses += - self.sft_coef* F.logsigmoid(self.beta * chosen_logratios)
            losses += - self.sft_coef * policy_chosen_logps
    ...
    def get_batch_logps(
        ...
        model_log_probs = logits.log_softmax(-1)

        if calculate_entropy:
            log_vocab_size = np.log(logits.shape[-1])
            batch_entropy = -(model_log_probs.exp() * model_log_probs).sum(-1)
            batch_entropy_normalized = (batch_entropy * loss_mask).sum(axis = -1)/(loss_mask.sum(-1)*log_vocab_size)
        else:
            batch_entropy_normalized = None

        per_token_logps = torch.gather(model_log_probs, dim=2, index=labels.unsqueeze(2)).squeeze(2)

        return (per_token_logps * loss_mask).sum(-1), loss_mask.sum(-1), batch_entropy_normalized
    ...
    def concatenated_forward(
        ...
        all_logps, size_completion, entropy_normalized = self.get_batch_logps(
            all_logits,
            concatenated_batch["concatenated_labels"],
            is_encoder_decoder = self.is_encoder_decoder,
            label_pad_token_id = self.label_pad_token_id,
            calculate_entropy = (self.use_mallows or self.output_reference_entropy),
        )
        ...
        if self.length_normalization == True:
        ...
        if entropy_normalized is not None:
            entropy = (entropy_normalized[:len_chosen]+entropy_normalized[len_chosen:])/2.
        else:
            entropy=None
        ...
        return (chosen_logps, rejected_logps, chosen_logits, rejected_logits, nll_loss, entropy)
    ...
    def get_batch_loss_metrics(
        ...
        if (
            ...
            reference_entropy = batch["reference_entropy"]
        else:
            with torch.no_grad():
                if self.ref_model is None:
                    with self.null_ref_context():
        ...
        if self.output_reference_entropy:
            reference_entropy = self.accelerator.gather_for_metrics(
                    (reference_entropy)
                )
            self.all_reference_entropy.append(reference_entropy.cpu())
